...
import { Student, OfferedClass, getOfferedClassRepository } from "registrar";

export async function classes(): Promise<OfferedClass[]> {
  return await getOfferedClassRepository().allClasses();
}
...
export async function classesForStudent(
  ...
  console.log(
    `classesForStudent ${util.inspect(student)} => ${util.inspect(ret)}`
  );
  return ret;
}

export async function updateStudentEnrolledClasses(
  studentid: any,
  codes: string[]
): Promise<any> {
  return await getOfferedClassRepository().updateStudentEnrolledClasses(
    studentid,
    codes
  );
}
